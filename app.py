  ...
  if st.button("Process Emails"):
    try:
      with st.container(height = 700):
        ...
        #st.dataframe(df)
        st.text_area("From", df.loc[-1, 'From'])
        st.text_area("Subject", df.loc[-1, 'Subject'])
        st.text_area("Body Summary", df.loc[-1, 'Body_summary'])
        st.text_area("Category", df.loc[-1, 'Final_Class'])
